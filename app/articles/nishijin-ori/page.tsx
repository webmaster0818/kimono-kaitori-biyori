    ...
    <>
      ...
      <section id="auction-data" className="py-10">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="text-xl md:text-2xl font-bold mb-6 flex items-center gap-2">
            <span className="w-1.5 h-8 bg-[#6b4c8a] rounded-full inline-block" />
            【実売データ】公開オークションでの西陣織（帯）のリアルな落札相場
          </h2>
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                  <tr><td className="border border-[#e8ddd0] px-3 py-2">西陣織 帯（全般）</td><td className="border border-[#e8ddd0] px-3 py-2">約11,094円</td><td className="border border-[#e8ddd0] px-3 py-2">130,000円</td></tr>
                  <tr><td className="border border-[#e8ddd0] px-3 py-2">袋帯</td><td className="border border-[#e8ddd0] px-3 py-2">約12,312円</td><td className="border border-[#e8ddd0] px-3 py-2">86,130円</td></tr>
              </tbody>
            </table>
          </div>
          <p className="text-xs text-gray-500 mt-2">
            出典：<a href="https://auctions.yahoo.co.jp/closedsearch/closedsearch/%E8%A5%BF%E9%99%A3%E7%B9%94%20%E5%B8%AF/0/" target="_blank" rel="noopener noreferrer" className="text-[#6b4c8a] underline">Yahoo!オークション 落札相場</a>（2026年6月取得）。参考：オークファン集計では西陣織 帯 全体の平均約33,181円です
          </p>
          <div className="bg-[#fdf6f6] border border-[#e8b4b4] rounded-lg p-4 mt-5 text-sm text-gray-700 leading-relaxed">
            ...
            上記は公開オークションで実際に売買された“実売価格”の集計です。買取業者の査定額（買取価格）とは異なり、一般に買取価格は実売価格より低くなります。1円〜の極端な安値は証紙なし・状態不良・入札不成立に近い例も含むため、価格帯の“幅”の参考としてご覧ください。西陣織は織元・証紙・作家により価値が大きく変わります。金額は取得時点のもので、相場は時期・状態により変動し、査定額を保証するものではありません。
          </div>
          <p className="text-xs text-gray-500 mt-3">
      ...
      <section id="condition" className="py-10 section-light">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="text-xl md:text-2xl font-bold mb-6 flex items-center gap-2">
            <span className="w-1.5 h-8 bg-[#6b4c8a] rounded-full inline-block" />
            状態別の買取価値（証紙・織元・保存状態でどう変わるか）
          </h2>
          <p className="text-sm md:text-base leading-relaxed mb-6">
            同じ西陣織の帯でも、証紙の有無・織元・保存状態によって査定額は大きく変わります。種類（袋帯・名古屋帯など）が同じでも、下表のような条件で評価が上下します。前述の実売データの“幅”も、多くはこの状態差・証紙差によって生まれています。
          </p>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              <thead>
              ...
              <tbody>
                {[
                  ["証紙あり（西陣織工業組合の証紙・織屋番号）", "◎ 大きくプラス", "メガネ型証紙と生産者番号があると、本物の西陣織であることと織元が明確になり、評価が安定します。"],
                  ["証紙なし", "△ 下がりやすい", "西陣織か否か・どの織元かの判別が難しく、慎重な査定になりがち。たとう紙や箱に証紙が残っていないか確認を。"],
                  ["有名織元（川島織物・龍村美術織物など）", "◎ 別格", "名門織元の帯は別格の評価を受けやすく、証紙・織ネームで確認できます。"],
                  ["袋帯（フォーマル需要）", "○〜◎ 安定", "礼装・準礼装に用いる袋帯は需要が安定しており、格調高い古典柄ほど評価されやすい傾向です。"],
                  ["未使用・美品", "◎/○ 高評価", "締めジワや汚れのない未使用・美品は最も高くなりやすい状態。保管ジワのみなら有利です。"],
                  ["金銀箔のスレ・劣化・シミ・折りジワ", "△ マイナス", "箔や金銀糸の傷み・変色、シミ、強い折りジワは減額対象。難の程度・範囲で幅があります。"],
                  ["全通柄（柄が全体に通る）", "○ 加点", "六通柄より柄付けの手間がかかるため、全通柄は評価されやすい傾向があります。"],
                  ["正絹であること", "◎/○ 前提", "正絹であることが評価の前提。化繊・ポリエステルの帯は正絹より大幅に評価が下がります。"],
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="bg-[#faf7f3] border border-[#e8ddd0] rounded-xl p-6 md:p-7">
            <h2 className="text-lg md:text-xl font-bold text-[#6b4c8a] mb-4 flex items-center gap-2">
              ...
              西陣織を売るなら｜状況別のおすすめ買取方法
            </h2>
            <p className="text-sm text-gray-700 leading-relaxed mb-5">
              西陣織は証紙・作家・状態によって評価が大きく変わります。あなたの状況に合わせて、最適な売り方を選びましょう。
            </p>
            <div className="grid md:grid-cols-2 gap-4">
              <Link href="/ranking/" className="block bg-white border border-[#e8ddd0] rounded-lg p-5 hover:border-[#6b4c8a] transition-colors group">
                ...
                <p className="text-xs text-gray-600">価値の高い西陣織は業者で査定額の差が出やすいため、おすすめ買取業者ランキングで複数社を比較するのが有利です。</p>
              </Link>
