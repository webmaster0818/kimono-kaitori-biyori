...
const datasetStructuredData = {"@context": "https://schema.org", "@type": "Dataset", "name": "振袖の実売相場データ（公開オークション落札相場）", "description": "振袖について、公開オークション（Yahoo!オークションの落札相場）で実際に売買が成立した落札件数・平均落札価格・最高落札価格を集計した実売データ（一次情報）です。買取業者の査定額ではなく、個人間取引で成立した実売価格の集計値です。", "creator": {"@type": "Organization", "name": "着物の買取びより", "url": "https://kimonokaitori-biyori.com"}, "publisher": {"@type": "Organization", "name": "着物の買取びより", "url": "https://kimonokaitori-biyori.com"}, "isBasedOn": "https://auctions.yahoo.co.jp/closedsearch/closedsearch/%E6%8C%AF%E8%A2%96/0/", "variableMeasured": ["落札件数", "平均落札価格", "最高落札価格"], "measurementTechnique": "公開オークションの落札相場ページの手動集計（推定値・出典不明の数値は不採用）", "temporalCoverage": "P180D", "dateModified": "2026-08-24", "url": "https://kimonokaitori-biyori.com/articles/furisode/#auction-data", "isAccessibleForFree": true};
  ...
  return (
    <>
      ...
      <section
        ...
        style={{
          backgroundImage:
            "linear-gradient(135deg, rgba(248,243,250,0.45), rgba(252,248,242,0.4)), url('/images/heroes/furisode.jpg')",
        ...
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="bg-white border-2 border-[#6b4c8a] rounded-xl p-5 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
              <div><span className="text-gray-500">落札件数（直近30日）</span><br /><span className="text-lg font-bold">2,771件</span></div>
              <div><span className="text-gray-500">平均落札価格</span><br /><span className="text-lg font-bold">約7,395円</span></div>
              <div><span className="text-gray-500">最高落札価格（過去約180日・単品）</span><br /><span className="text-lg font-bold">330,000円</span><br /><span className="text-xs text-gray-500">作家物 振袖(単品)</span></div>
            </div>
            <p className="text-xs text-gray-500 mt-3">出典: オークファン（aucfan.com）の振袖落札相場ページ・Yahoo!オークション落札相場（2026年8月24日取得）。検索ノイズ(刀剣用語・玩具・まとめ売り)を除外した実測値です(総絞りは275,000円)。以下の区分別テーブルは前回集計（取得日記載）です。</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                  <tr><td className="border border-[#e8ddd0] px-3 py-2">振袖（全般）</td><td className="border border-[#e8ddd0] px-3 py-2">約3,982円</td><td className="border border-[#e8ddd0] px-3 py-2">1,632,500円</td></tr>
                  <tr><td className="border border-[#e8ddd0] px-3 py-2">中古振袖（参考）</td><td className="border border-[#e8ddd0] px-3 py-2">約5,627円</td><td className="border border-[#e8ddd0] px-3 py-2">－</td></tr>
              </tbody>
            </table>
          </div>
          <p className="text-xs text-gray-500 mt-2">
            出典：<a href="https://auctions.yahoo.co.jp/closedsearch/closedsearch/%E6%8C%AF%E8%A2%96/0/" target="_blank" rel="noopener noreferrer" className="text-[#6b4c8a] underline">Yahoo!オークション 落札相場</a>（2026年6月取得）。最高額は新品同様・作家物などの例で、平均は中古の安価な出品も多く含みます
          </p>
          <div className="bg-[#fdf6f6] border border-[#e8b4b4] rounded-lg p-4 mt-5 text-sm text-gray-700 leading-relaxed">
            ...
            上記は公開オークションで実際に売買された“実売価格”の集計です。買取業者の査定額（買取価格）とは異なり、一般に買取価格は実売価格より低くなります。1円〜の極端な安値は証紙なし・状態不良・入札不成立に近い例も含むため、価格帯の“幅”の参考としてご覧ください。振袖は正絹か否か・作家・証紙・サイズ・柄の人気で価値が大きく変わります（総絞り・有名作家・アンティークは高く、ポリエステルや状態不良は安値も多い）。金額は取得時点のもので、相場は時期・状態により変動し、査定額を保証するものではありません。
          </div>
          <p className="text-xs text-gray-500 mt-3">
      ...
      <section id="condition" className="py-10 section-light">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="text-xl md:text-2xl font-bold mb-6 flex items-center gap-2">
            <span className="w-1.5 h-8 bg-[#6b4c8a] rounded-full inline-block" />
            状態別の買取価値（作家・素材・保存状態でどう変わるか）
          </h2>
          <p className="text-sm md:text-base leading-relaxed mb-6">
            同じ振袖でも、作家・素材・技法・保存状態によって査定額は大きく変わります。ランクやブランドが同じでも、下表のような条件で評価が上下します。前述の実売データの“幅”も、多くはこの状態差によって生まれています。
          </p>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              <thead>
              ...
              <tbody>
                {[
                  ["作家物・落款あり", "◎ 別格", "著名作家の手描き友禅など、落款のある作品は別格の評価。証紙・落款で確認できます。"],
                  ["総絞り・本絞り（手の込んだ技法）", "◎ 高評価", "総絞りなど手間のかかる技法は希少性が高く、高い評価を受けやすい傾向です。"],
                  ["正絹で絵羽・金彩・刺繍が上質", "◎ 高評価", "正絹で絵羽づけがきれいに繋がり、金彩や刺繍の質が高いものは評価が上がります。"],
                  ["有名ブランド・老舗呉服店の証紙/証明", "○〜◎", "有名ブランドや老舗の証紙・証明があると、産地や品質が明確になり評価が安定します。"],
                  ["未使用・しつけ糸付き", "◎ 高評価", "新品同様は最も高くなりやすい状態。しつけ糸が残っていれば特に有利です。"],
                  ["美品（着用少・難なし）", "○ 良好", "着用が少なく目立つシミ・スレがなければ、安定した評価が期待できます。"],
                  ["仕立て上がり（裄・身丈などサイズ）", "○〜△", "サイズで変動。裄・身丈が大きめだと現代向けで有利、寸法が小さいと下がる場合も。"],
                  ["シミ・金彩や刺繍の劣化・変色・古い柄", "△ マイナス", "状態不良や金彩・刺繍の劣化、変色は減額対象。振袖は流行の影響を受けやすく、古い柄も下がりやすい傾向です。"],
